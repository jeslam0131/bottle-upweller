# hsv_roi_grid_dualcolor_lmi_weighted_known_tracer_fittoscreen.py
# -------------------------------------------------------------------
# • Pick video (File Explorer) → draw polygon ROI (click to add; drag vertices; 'c' to start)
# • Fixed HSV thresholds (set below); 5×8 grid inside polygon
# • Per-tile % overlays for Color1 (Green) and Color2 (Magenta)
# • LMI (weighted) computed from the tracer color ONLY using a known tracer fraction
# • Crisp text (solid background boxes)
# • Windows are RESIZABLE and auto-scale to fit the current screen (no clipping)
# • Playback at real-time × SPEED_MULTIPLIER (fixed in code; no hotkeys)
#
...
# Set these so Color1 (green tint) catches your "black" particles, and Color2 (magenta) your "white".
COLOR1_HSV_LO = (0,   0,  100)    # white (example)
COLOR1_HSV_HI = (179, 255, 255)
COLOR2_HSV_LO = (0,   0,    0)    # black (example)
COLOR2_HSV_HI = (179, 255, 100)

# Which color is the TRACER for LMI?
TRACER_IS = "C2"                  # "C1" or "C2"

# Known global tracer fraction (e.g., 0.20 = 20% tracer by volume/mass)
KNOWN_TRACER_FRACTION = 0.20

# Overlay styling
TINT1_BGR  = (0, 255, 0)          # C1 tint (green)
TINT2_BGR  = (255, 0, 255)        # C2 tint (magenta)
TINT_ALPHA = 0.15                 # 0..1 light overlay
...
GRID_THICKNESS = 2
MIN_TILE_COVER_FRAC = 0.15        # skip tiny-coverage tiles

# Playback pacing (fixed)
SPEED_MULTIPLIER = 1.0            # 1.0 = real-time;
...
FIT_TO_SCREEN = True
# Fallback caps if OS query fails (edit if desired):
DEFAULT_MAX_W, DEFAULT_MAX_H = 1600, 900
# ================================================================ #

FONT = cv2.FONT_HERSHEY_SIMPLEX

# ---------- Display helpers (scale to fit screen, keep aspect) ---------- #
def get_screen_size():
    """Return (screen_width, screen_height) in pixels; fall back if unavailable."""
    try:
        import ctypes
        user32 = ctypes.windll.user32
        # Make DPI aware so metrics are in real pixels
        try:
...
# ---------- Crisp label with solid background (no blur) ---------- #
def put_label(img, x, y, text, fg=(255,255,255), bg=(0,0,0), scale=0.7, thick=2, pad=3):
    x = int(x); y = int(y)
...
def pick_video_path() -> str:
    try:
        ...
        if not path or not os.path.exists(path):
            raise SystemExit("No valid video selected.")
        return path
    except Exception as e:
        raise SystemExit(f"File dialog error: {e}\nTip: ensure tkinter is available (conda: conda install tk).")
    ...
    def draw_overlay(self, img):
        ...
        if len(self.points) >= 1:
            for i, p in enumerate(self.points):
                ...
                if i > 0:
                    cv2.line(out, self.points[i-1], p, (0, 255, 0), 2, cv2.LINE_AA)
...
def analyze_tiles_tracer(sub_tracer: np.ndarray, valid_sub: np.ndarray,
    ...
    Returns:
      tiles: list of dicts {rect, n, p_tracer}
      p_meas: measured tracer fraction over ROI (sanity check)
      p_w, s2_w: weighted mean and population weighted variance of per-tile fractions
    ...
    for j in range(gh):
        for i in range(gw):
            ...
            t_tr = sub_tracer[y0:y1, x0:x1]
            p_t = float(t_tr[vmask].mean()) / 255.0
    ...
    s2_w  = float((w * (p - p_w) ** 2).sum() / wsum)  # population weighted variance
...
def lacey_index_weighted_known_p(s2_w: float, p_known: float, m_bar: float) -> float:
    """
    Lacey MI with known global tracer fraction p_known:
      L = (s0^2 - s_w^2) / (s0^2 - s_r^2),
      where s0^2 = p_known (1 - p_known),  s_r^2 = s0^2 / m_bar.
    """
...
def main():
    ...
    # ROI drawing (RESIZABLE; scaled to fit screen)
    draw_win = "ROI (draw polygon, 'c' to start)"
    cv2.namedWindow(draw_win, cv2.WINDOW_NORMAL)
    roi = PolygonROI(draw_win)
    roi.install()
    ...
    while True:
        ...
        for line in help_lines:
            put_label(vis, 10, y, line, fg=(255,255,255), bg=(0,0,0), scale=0.6, thick=2)
            y += 26

        if FIT_TO_SCREEN:
            scr_w, scr_h = get_screen_size()
            vis_disp, _ = scale_to_fit(vis, scr_w - 40, scr_h - 80)
    ...
    # Playback (RESIZABLE; scaled to fit screen)
    cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
    play_win = "Playback (fixed params)"
    ...
    while True:
        ...
        # ---- per-tile analysis (tracer only) + weighted LMI with known p ----
        ...
        # ---- overlay per-tile % for each color (verification) ----
        for t in tiles:
            x0, y0, x1, y1 = t["rect"]
        ...
        if SHOW_LMI:
            ...
            lmi_txt = (f"Lacey MI (weighted, tracer={which}, p_known={KNOWN_TRACER_FRACTION:.2f}): "
                       f"{L:.3f}   m̄={m_bar:.0f}   tiles={used}")
            put_label(left, 18, 36, lmi_txt, fg=LABEL_FG, bg=LABEL_BG, scale=0.7, thick=2)
            put_label(left, 18, 66, f"Measured tracer p̂={p_meas:.3f} (sanity; not used)",
                      fg=LABEL_FG, bg=LABEL_BG, scale=0.7, thick=2)
        ...
        if FIT_TO_SCREEN:
            scr_w, scr_h = get_screen_size()
            stacked_disp, _ = scale_to_fit(stacked, scr_w - 40, scr_h - 80)
            cv2.imshow(play_win, stacked_disp)
            cv2.moveWindow(play_win, 40, 40)
        else:
